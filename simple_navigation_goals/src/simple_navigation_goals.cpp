...
bool isSubInitPose(){
  for(int i=0; i<2; i++){
    if(fabs(mPose[i]-mAmclPose[i]) > 0.03){
      return false;
    }
  }
...
void pubInitialPose(ros::NodeHandle nh){
...
double** getOdom(){
  ...
  // double list[getTargetSize()][MAX_ARR_SIZE];
  ...
  if(inFile.is_open()){
    ...
    while(getline(inFile, inLine)){
      if(inLine.at(0) == '%'){
        ...
        // std::stringstream ssDouble(inLine);
...
int main(int argc, char** argv){
  ...
  ROS_INFO("waiting for inital position setting");
  ...
  // if(!isSubInitPose()){
  //   ROS_INFO("Error at Sending Initial Position, Shutdown this Program");
  //   return 0;
  // }
